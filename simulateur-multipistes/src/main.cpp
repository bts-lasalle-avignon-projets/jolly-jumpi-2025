...
int  nbPistes   = NB_PISTES; //!< le nombre de pistes détectables
...
void setup()
{
    ...
    Serial.println("[main] Nb pistes     : " + String(NB_PISTES));
...
void loop()
{
    ...
    if(tirEncours)
    {
        ...
        if(tir >= 1 && tir <= NB_TROUS)
        {
            ...
            int           numeroTable  = random(0, long(nbPistes)) + 1;
            envoyerTrameTir(numeroTable, score);
            sprintf(strMessageDisplay,
                    "Piste %d - Trou %d - score %d",
                    numeroTable,
                    tir,
                    score);
        ...
        switch(typeTrame)
        {
            ...
            case TypeTrame::MODE_JEU:
                ...
                if(etatPartie == Finie && estAssocie)
                {
                    ...
                    nbPistes = trame.charAt(4) - '0';
                    if(nbPistes < 1 || nbPistes > NB_PISTES)
                        nbPistes = NB_PISTES;
                    digitalWrite(GPIO_LED_ROUGE, LOW);
                    ...
                    Serial.println("[main] Nb joueurs : " + String(nbPistes));
#endif
                }
                break;
